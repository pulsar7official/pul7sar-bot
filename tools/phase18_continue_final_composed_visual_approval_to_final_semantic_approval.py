#!/usr/bin/env python3
"""Continue one exact CS345 checkpoint into existing CS282 Final Semantic Approval."""
...
from engine.intelligence.qwen_image_final_composed_visual_approval_to_final_semantic_approval import (
    continue_final_composed_visual_approval_to_final_semantic_approval,
)


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--cs345-receipt", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--repo-root", default=Path.cwd(), type=Path)
    args = parser.parse_args()

    run = continue_final_composed_visual_approval_to_final_semantic_approval(
        args.cs345_receipt,
        args.output_dir,
