#!/usr/bin/env python3
"""Continue exact CS349 Genuine Golden materialization through existing CS286 readiness."""
...
from engine.intelligence.qwen_image_genuine_golden_materialization_to_publication_readiness import (
    continue_genuine_golden_materialization_to_publication_readiness,
)


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--cs349-receipt", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--repo-root", default=Path.cwd(), type=Path)
    args = parser.parse_args()
    run = continue_genuine_golden_materialization_to_publication_readiness(
        args.cs349_receipt,
        args.output_dir,
