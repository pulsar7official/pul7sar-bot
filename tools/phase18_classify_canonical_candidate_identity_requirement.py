...
def main() -> int:
    parser = argparse.ArgumentParser(description="Classify whether a CS264 candidate requires separate pixel-identity review.")
    parser.add_argument("--cs264-receipt", required=True)
    parser.add_argument("--cs257-run-dir", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--repo-root", default=".")
    args = parser.parse_args()
    result = run_identity_requirement(
        Path(args.cs264_receipt), Path(args.cs257_run_dir), Path(args.output_dir), repo_root=Path(args.repo_root)
    )
