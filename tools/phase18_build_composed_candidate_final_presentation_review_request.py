#!/usr/bin/env python3
"""Build/verify CS279 without accepting presentation verdicts or publication authority."""
...
def main() -> int:
    ...
    if not all((args.cs278_receipt, args.output_dir)):
        parser.error("build mode requires --cs278-receipt --output-dir")
    path = build_composed_candidate_final_presentation_review_request(
        args.cs278_receipt, args.output_dir, repo_root=REPO_ROOT
    )
    receipt = verify_composed_candidate_final_presentation_review_request(path, repo_root=REPO_ROOT)
    print(json.dumps(receipt, ensure_ascii=False, indent=2))
    return 0
