"""CS284 v2: execute SemanticPublicationGate from exact lineage-bound evidence.

CS309 hardens this stage so an external evidence envelope cannot be substituted across
runs.  The exact CS283 request, its exact CS282 parent, composed PNG, generation context,
and zero-cost/offline verifier lineage must all match before the repository gate runs.
The same checks are replayed during verification.  Genuine-Golden creation and
publication readiness remain closed for downstream authority.
"""
...
SCHEMA = "pul7sar-phase18-qwen-image-composed-candidate-semantic-publication-execution-v2"
...
def _verifier(raw: Mapping[str, Any]) -> VisionVerifierProfile:
    try:
        capabilities = frozenset(VisionVerificationCapability(item) for item in raw.get("capabilities", ()))
    except ValueError as exc:
        raise ValueError("QWEN_SEMANTIC_PUBLICATION_EXECUTION_VERIFIER_CAPABILITY_INVALID") from exc
...
) -> tuple[GenerationPackage, BaseSceneEvidence, VisionVerifierProfile]:
    assert_lineage_bound_semantic_publication_evidence(raw, cs283=cs283, cs283_binding=cs283_binding)
    package_raw = raw.get("generation_package")
    base_raw = raw.get("base_scene_evidence")
    verifier_raw = raw.get("vision_verifier_profile")
    if not all(isinstance(item, Mapping) for item in (package_raw, base_raw, verifier_raw)):
        raise ValueError("QWEN_SEMANTIC_PUBLICATION_EXECUTION_INPUTS_INVALID")
    return _package(package_raw), _evidence(base_raw), _verifier(verifier_raw)
...
def execute_semantic_publication_gate(cs283_receipt_path: Path, evidence_path: Path, output_dir: Path, *, repo_root: Path) -> Path:
    if output_dir.exists() or not output_dir.parent.is_dir():
        raise ValueError("QWEN_SEMANTIC_PUBLICATION_EXECUTION_OUTPUT_INVALID")
    ...
    if cs283.get("schema") != CS283_SCHEMA or cs283.get("semantic_publication_execution_requested") is not True:
        raise ValueError("QWEN_SEMANTIC_PUBLICATION_EXECUTION_CS283_STATE_INVALID")
    ...
    package, base, verifier = _validated_inputs(raw=raw, cs283=cs283, cs283_binding=cs283_binding)
    ...
    receipt: dict[str, Any] = {
        ...
        "policy": {
            "decision_must_come_from_repository_semantic_publication_gate": True,
            "external_allowed_override_forbidden": True,
            "evidence_must_bind_exact_cs283_cs282_png_and_generation_context": True,
            "zero_cost_offline_verifier_reasserted_before_gate": True,
            "gate_success_does_not_create_genuine_golden_png": True,
            "gate_success_does_not_set_publication_ready": True,
        },
    }
    receipt["receipt_sha256"] = sha256_json(receipt)
    output_dir.mkdir(mode=0o700)
    path = output_dir / "semantic_publication_execution.json"
    tmp = output_dir / ".semantic_publication_execution.json.tmp"
    try:
...
def verify_semantic_publication_execution(receipt_path: Path, *, repo_root: Path) -> dict[str, Any]:
    ...
    if receipt.get("schema") != SCHEMA or receipt.get("status") != STATUS or claimed != sha256_json(unsigned):
        raise ValueError("QWEN_SEMANTIC_PUBLICATION_EXECUTION_RECEIPT_INVALID")
    source = receipt.get("source_cs283_semantic_publication_request")
    if not isinstance(source, Mapping):
        raise ValueError("QWEN_SEMANTIC_PUBLICATION_EXECUTION_CS283_BINDING_INVALID")
    ...
    if source.get("receipt_sha256") != cs283.get("receipt_sha256"):
        raise ValueError("QWEN_SEMANTIC_PUBLICATION_EXECUTION_CS283_RECEIPT_DRIFT")
    current_cs283_binding = _bind(repo_root, cs283_path, "QWEN_SEMANTIC_PUBLICATION_EXECUTION_CS283_INVALID")
    evidence_binding = receipt.get("semantic_publication_execution_evidence")
    if not isinstance(evidence_binding, Mapping):
        raise ValueError("QWEN_SEMANTIC_PUBLICATION_EXECUTION_EVIDENCE_BINDING_INVALID")
    evidence_path = _reopen(repo_root, evidence_binding, "QWEN_SEMANTIC_PUBLICATION_EXECUTION_EVIDENCE_INVALID")
    raw = _json(evidence_path, "QWEN_SEMANTIC_PUBLICATION_EXECUTION_EVIDENCE_INVALID")
    package, base, verifier = _validated_inputs(raw=raw, cs283=cs283, cs283_binding=current_cs283_binding)
    decision = SemanticPublicationGate().evaluate(package, base, verifier)
    expected = {
        "story_snapshot_sha256": cs283["story_snapshot_sha256"],
        "composed_candidate_png": dict(cs283["composed_candidate_png"]),
        "generation_context": dict(cs283["generation_context"]),
        "weighted_score": cs283["weighted_score"],
        "quality_tier": cs283["quality_tier"],
        "composed_visual_approved": True,
        "semantic_approved": True,
        "semantic_publication_execution_requested": True,
        "semantic_publication_gate_executed": True,
        "semantic_publication_allowed": bool(decision.allowed),
        "base_scene_accepted": bool(decision.base_scene_accepted),
        "semantic_verifier_eligible": bool(decision.semantic_verifier_eligible),
        "semantic_publication_failures": list(decision.failures),
        "semantic_publication_warnings": list(decision.warnings),
        "genuine_golden_png_created": False,
        "publication_ready": False,
    }
    for field, value in expected.items():
        if receipt.get(field) != value:
            raise ValueError(f"QWEN_SEMANTIC_PUBLICATION_EXECUTION_STATE_DRIFT:{field}")
    return receipt
