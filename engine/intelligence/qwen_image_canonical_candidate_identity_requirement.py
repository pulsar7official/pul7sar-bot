...
Change Set 387 additionally preserves the exact replay-verified CS351
CUDA/native-BF16 static-readiness receipt binding already carried by CS360.
Verification freshly replays CS360 and compares that readiness binding before
identity requirement classification can be trusted downstream.
...
SCHEMA = "pul7sar-phase18-qwen-image-canonical-candidate-identity-requirement-v3"
...
def _readiness_lineage(source: Mapping[str, Any]) -> dict[str, Any]:
    if source.get("static_readiness_receipt_verified") is not True:
        raise ValueError("QWEN_IDREQ_READINESS_AUTHORITY_MISSING")
    readiness = source.get("static_readiness_receipt")
    if not isinstance(readiness, Mapping):
        raise ValueError("QWEN_IDREQ_READINESS_MISSING")
    ...
    if (
        ...
        raise ValueError("QWEN_IDREQ_READINESS_PATH_INVALID")
    if not _is_sha256(digest):
        raise ValueError("QWEN_IDREQ_READINESS_DIGEST_INVALID")
    if isinstance(size, bool) or not isinstance(size, int) or size <= 0:
        raise ValueError("QWEN_IDREQ_READINESS_BYTE_SIZE_INVALID")
    return {
...
def verify_identity_requirement(receipt_path: Path, *, repo_root: Path) -> dict[str, Any]:
    ...
    expected_readiness = _readiness_lineage(source)
    if receipt.get("static_readiness_receipt_verified") is not True:
        raise ValueError("QWEN_IDREQ_READINESS_AUTHORITY_MISSING")
    if receipt.get("static_readiness_receipt") != expected_readiness:
        raise ValueError("QWEN_IDREQ_READINESS_RECEIPT_DRIFT")
