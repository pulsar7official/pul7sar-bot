...
artifact integrity, semantic visual inspection, hybrid layer QA, semantic
publication verification, Golden visual quality, and final export authorization
to all agree before a visual may be called publication-ready.
"""
...
class PublicationReadinessEvidence:
    ...
    artifact_integrity: HybridArtifactIntegrityDecision
...
class PublicationReadinessGate:
    ...
    def evaluate(self, evidence: PublicationReadinessEvidence) -> PublicationReadinessDecision:
        ...
        if not evidence.artifact_integrity.valid:
            blockers.extend("artifact_integrity:" + item for item in evidence.artifact_integrity.failures)
