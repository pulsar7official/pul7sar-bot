...
This postflight closes a stale-result gap in the one-command GPU path: a durable
job reaching ``succeeded`` and pointing at a PNG is not enough evidence by
itself. The exact executor result and proof metadata must still replay against
the locked Candidate 1 identity and the current PNG bytes before the result may
