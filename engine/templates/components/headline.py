...
def _resolve_headline_font(render_context: RenderContext) -> Path:
    ...
    path = fonts.get("arabic") or fonts.get("headline") or fonts.get("bold") or fonts.get("fallback")
    if path is None:
        raise TemplateError("No usable headline font was resolved")
    return Path(path)

def headline_component(render_context: RenderContext, width: int, height: int) -> List[Layer]:
    del width, height
    ...
    fitted = fit_headline(
        content.headline, str(_resolve_headline_font(render_context)),
        max_width=HEADLINE_RIGHT - HEADLINE_LEFT,
        max_height=HEADLINE_BOTTOM - HEADLINE_TOP,
        max_lines=HEADLINE_MAX_LINES, min_font_size=30, max_font_size=58,
    )
    ...
    total_h = sum(img.height for img in rendered_lines) + line_gap * max(0, len(rendered_lines)-1)
    ...
    for line_image in rendered_lines:
        ...
        layers.append(Layer(
            ...
            properties={
                "image": line_image, "x": max(HEADLINE_LEFT, x), "y": y,
                "width": line_image.width, "height": line_image.height,
            },
        ))
