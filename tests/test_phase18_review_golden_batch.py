...
from tools.phase18_review_golden_batch import evaluate


class ReviewGoldenBatchTests(unittest.TestCase):
    def write_inputs(self, root, reviews):
        root = Path(root)
        execution = {
        ...
        review_path.write_text(json.dumps({"candidates": reviews}), encoding="utf-8")
