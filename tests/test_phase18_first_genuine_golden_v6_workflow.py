...
class FirstGenuineGoldenV6WorkflowTests(unittest.TestCase):
    ...
    def test_canonical_workflow_is_manual_self_hosted_and_immutable(self):
        ...
        self.assertIn("phase18_preflight_semantic_gpu.py", text)
    ...
    def test_immutable_qwen_semantic_preflight_precedes_flux_candidate_execution(self):
        text = Path(".github/workflows/phase18-first-genuine-golden-v6.yml").read_text(encoding="utf-8")
        semantic = text.index("Preflight immutable Qwen semantic runtime and snapshot before FLUX")
        ...
        self.assertLess(semantic, execute)
        ...
        self.assertIn("resolved_snapshot_revision", text)
        self.assertIn("revision_pinned", text)
        self.assertIn("semantic_runtime_ready", text)
        self.assertIn("semantic_model_ready", text)
        self.assertIn("generation_authorized", text)
        self.assertIn("queue_mutated", text)
        self.assertIn("png_created", text)
