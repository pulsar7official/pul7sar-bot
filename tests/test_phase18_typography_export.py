...
from engine.intelligence.post_composition import AssetIntegrityRecord, PostCompositionPlanner
from engine.intelligence.typography import DeterministicTypographyEngine, FontReference, Pul7sarTypographyPolicy, TextBox, TextRole
...
class TypographyExportTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.package = GenerationPackage(
            platform="instagram_feed", canvas="1080x1350", scene_prompt="base scene",
            negative_constraints=(), asset_ids=("logo", "pulse", "crest", "instagram"), factual_constraints=(),
            layout_boxes={
            ...
            }, accent_hex="#EF0107", metadata={},
        )
        self.assets = AssetBundle((
            AssetReference("logo", AssetRole.PUL7SAR_LOGO, AssetTreatment.EXACT, metadata={"sha256": self.LOGO_SHA}),
        ...
        self.integrity = (AssetIntegrityRecord("logo", self.LOGO_SHA),)
    ...
    def test_headline_fits_without_silent_truncation(self):
        ...
        self.assertFalse(layout.truncated); self.assertLessEqual(len(layout.lines), 3)

    def test_overlong_headline_fails_closed(self):
        with self.assertRaises(ValueError):
            self.engine.fit("This headline cannot possibly fit safely inside this tiny box", TextBox(0, 0, 80, 40), self.headline_style)

    def test_score_is_single_line(self):
        self.assertEqual(len(self.engine.fit("3 - 1", self.text_box("score"), self.score_style).lines), 1)

    def test_social_footer_is_compact_single_line(self):
        self.assertEqual(len(self.engine.fit("PUL7SAR", self.text_box("social_footer"), self.footer_style).lines), 1)

    def test_export_authorizes_complete_valid_output(self):
        plan = PostCompositionPlanner().compile(
            self.package, self.assets, headline="Arsenal move closer to the deal", score="3 - 1",
            social_handle="PUL7SAR", integrity_records=self.integrity,
        )
        layouts = (
        ...
        output = FinalComposedOutput("instagram_feed", "1080x1350", "base-scene", "composed-final", layouts)
        auth = FinalExportGate().authorize(
            self.package, self.assets, plan, output,
            approved_styles={TextRole.HEADLINE.value: self.headline_style, TextRole.SCORE.value: self.score_style, TextRole.SOCIAL_FOOTER.value: self.footer_style},
        )
        self.assertTrue(auth.allowed, auth.failures)
        self.assertIsNotNone(auth.token)

    def test_export_rejects_missing_rendered_headline(self):
        plan = PostCompositionPlanner().compile(
            self.package, self.assets, headline="Arsenal move closer to the deal", social_handle="PUL7SAR", integrity_records=self.integrity,
        )
        output = FinalComposedOutput(
            "instagram_feed", "1080x1350", "base-scene", "composed-final",
            (self.engine.fit("PUL7SAR", self.text_box("social_footer"), self.footer_style),),
        )
        auth = FinalExportGate().authorize(self.package, self.assets, plan, output, approved_styles={TextRole.SOCIAL_FOOTER.value: self.footer_style})
    ...
    def test_export_rejects_wrong_text_geometry(self):
        plan = PostCompositionPlanner().compile(self.package, self.assets, headline="Arsenal move closer", integrity_records=self.integrity)
        ...
        auth = FinalExportGate().authorize(self.package, self.assets, plan, output, approved_styles={TextRole.HEADLINE.value: self.headline_style})
        self.assertFalse(auth.allowed); self.assertIn("text geometry mismatch: headline", auth.failures)
