...
class Phase18GpuSmokeWorkflowTests(unittest.TestCase):
    ...
    def test_repository_integrity_is_fail_closed_before_any_gpu_probe_or_dependency_work(self):
        ...
        cache_budget = self.text.index("python tools/phase18_preflight_first_golden_cache_budget.py")
        ...
        self.assertLess(repo_gate, cache_budget)
        self.assertLess(repo_gate, semantic)
        self.assertLess(repo_gate, generation)

    def test_combined_cache_budget_is_fail_closed_before_qwen_or_flux_downloads(self):
        self.assertIn("tools/phase18_preflight_first_golden_cache_budget.py", self.text)
        self.assertIn("first-golden-cache-budget.json", self.text)
        self.assertIn("pul7sar-first-golden-cache-budget-v1", self.text)
        self.assertIn("Qwen/Qwen2.5-VL-3B-Instruct", self.text)
        self.assertIn("black-forest-labs/FLUX.2-klein-4B", self.text)
        self.assertIn('payload.get("downloads_performed") is not False', self.text)
        self.assertIn('budget.get("eligible") is not True', self.text)
        ...
        generation = self.text.index("python tools/phase18_first_png.py")
        self.assertLess(dependency_install, cache_budget)
        self.assertLess(cache_budget, semantic)
        self.assertLess(cache_budget, flux_prefetch)
        self.assertLess(cache_budget, generation)
    ...
    def test_builds_and_replays_repository_cache_semantic_generation_and_postflight_evidence_before_upload(self):
        ...
        self.assertIn("--include output/phase18_gpu_smoke/first-golden-cache-budget.json", self.text)
