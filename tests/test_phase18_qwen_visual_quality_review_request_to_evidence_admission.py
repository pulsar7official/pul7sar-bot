...
STORY_SHA = "a" * 64
INVENTORY_SHA = "b" * 64
MODEL_REVISION = "qwen-image-phase18-pinned"
...
class Phase18VisualQualityReviewRequestToEvidenceAdmissionTests(unittest.TestCase):
    def _fixture(self, root: Path):
        ...
        candidate = {**bind(c, root), "width": 4, "height": 4}
        composed = {**bind(p, root), "width": 4, "height": 4}
        cs338v = {
            "schema": cs339.CS338_SCHEMA,
            "status": "VISUAL_QUALITY_REVIEW_REQUEST_READY",
            "story_snapshot_sha256": STORY_SHA,
            "candidate_png": candidate,
            "composed_candidate_png": composed,
            "cs274_receipt": bind(r274, root),
        ...
        cs275v = {
            "schema": cs339.CS275_SCHEMA,
            "story_snapshot_sha256": STORY_SHA,
            "composed_candidate_png": composed,
            "source_cs274_request": {**bind(r274, root), "receipt_sha256": cs274v["receipt_sha256"]},
            "external_review_evidence": bind(ext, root),
            "visual_quality_review_requested": True,
            "visual_quality_review_executed": True,
            "visual_quality_evidence_admitted": True,
            **closed(),
        }
        return r338, r274, ext, cs338v, cs274v, cs275v

    def _build(self, root: Path, *, cs338_override: dict | None = None):
        r338, _r274, ext, cs338v, cs274v, cs275v = self._fixture(root)
        if cs338_override:
            cs338v.update(cs338_override)

        def build(_req, _ext, out, *, repo_root):
            out.mkdir()
            q = out / "composed_candidate_visual_quality_review_evidence.json"
            q.write_text("275\n", encoding="utf-8")
            return q

        patches = (
            patch.object(cs339, "verify_hybrid_surface_semantic_qa_to_visual_quality_review_request", return_value=cs338v),
            patch.object(cs339, "verify_composed_candidate_visual_quality_review_request", return_value=cs274v),
            patch.object(cs339, "build_composed_candidate_visual_quality_review_evidence", side_effect=build),
            patch.object(cs339, "verify_composed_candidate_visual_quality_review_evidence", return_value=cs275v),
        )
        with patches[0], patches[1], patches[2], patches[3]:
            run = cs339.continue_visual_quality_review_request_to_evidence_admission(r338, ext, root / "out", repo_root=root)
        return run, cs338v, cs274v, cs275v

    def test_exact_cs338_to_cs275_path_preserves_snapshot_and_stops_before_cs276(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            run, _cs338v, _cs274v, _cs275v = self._build(root)
    ...
    def test_unverified_snapshot_is_rejected_before_cs275_construction(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            with self.assertRaisesRegex(ValueError, "CS339_CS338_SNAPSHOT_BYTE_INVENTORY_UNVERIFIED"):
                self._build(root, cs338_override={"snapshot_byte_inventory_verified": False})
            self.assertFalse((root / "out").exists())

    def test_inventory_sha_tampering_is_rejected_after_outer_digest_recomputed(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            run, cs338v, cs274v, cs275v = self._build(root)
            receipt = cs339._json(run.receipt_path, "bad")
            receipt["snapshot_inventory_sha256"] = "c" * 64
            unsigned = dict(receipt); unsigned.pop("receipt_sha256", None)
            receipt["receipt_sha256"] = cs339.sha256_json(unsigned)
            run.receipt_path.write_text(json.dumps(receipt, separators=(",", ":")) + "\n", encoding="utf-8")
            with patch.object(cs339, "verify_hybrid_surface_semantic_qa_to_visual_quality_review_request", return_value=cs338v), patch.object(cs339, "verify_composed_candidate_visual_quality_review_request", return_value=cs274v), patch.object(cs339, "verify_composed_candidate_visual_quality_review_evidence", return_value=cs275v):
                with self.assertRaisesRegex(ValueError, "CS339_CS338_SNAPSHOT_LINEAGE_DRIFT:snapshot_inventory_sha256"):
                    cs339.verify_visual_quality_review_request_to_evidence_admission(run.receipt_path, repo_root=root)

    def test_model_revision_tampering_is_rejected_after_outer_digest_recomputed(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            run, cs338v, cs274v, cs275v = self._build(root)
            receipt = cs339._json(run.receipt_path, "bad")
            ...
            run.receipt_path.write_text(json.dumps(receipt, separators=(",", ":")) + "\n", encoding="utf-8")
            with patch.object(cs339, "verify_hybrid_surface_semantic_qa_to_visual_quality_review_request", return_value=cs338v), patch.object(cs339, "verify_composed_candidate_visual_quality_review_request", return_value=cs274v), patch.object(cs339, "verify_composed_candidate_visual_quality_review_evidence", return_value=cs275v):
                with self.assertRaisesRegex(ValueError, "CS339_CS338_SNAPSHOT_LINEAGE_DRIFT:model_revision"):
                    cs339.verify_visual_quality_review_request_to_evidence_admission(run.receipt_path, repo_root=root)
