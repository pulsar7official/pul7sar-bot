...
from engine.intelligence.premium_visual_quality_gate import (
    PremiumVisualEvidence,
    PremiumVisualQualityGate,
    VisualQualityDisposition,
)
...
class PremiumVisualQualityGateTests(unittest.TestCase):
    ...
    def evidence(self, **kwargs):
        data = dict(
            ...
            concept=VisualConceptArchetype.DECISIVE_MOMENT,
            ...
            verified_stronger_moment_available=True,
            score_monument_used=False,
            ...
            photographic_context_used=True,
            context_is_story_evidence=True,
        )
        data.update(kwargs)
    ...
    def test_clean_moment_led_candidate_reaches_human_review_not_publication(self):
        decision = self.gate.evaluate(self.evidence(), concept_decision=self.concept())
    ...
    def test_duplicate_pulse_and_unexplained_portal_are_hard_blockers(self):
        decision = self.gate.evaluate(self.evidence(
            unexplained_graphic_panel_count=1,
            decorative_pulse_count_outside_brand=1,
        ))
    ...
    def test_score_monument_is_blocked_when_verified_stronger_moment_exists(self):
        decision = self.gate.evaluate(self.evidence(
    ...
    def test_unnecessary_full_pitch_and_oversized_brand_are_blocked(self):
        decision = self.gate.evaluate(self.evidence(
            full_pitch_visible=True,
            pitch_is_information=False,
            brand_width_ratio=0.42,
            brand_height_ratio=0.15,
        ))
        self.assertEqual(decision.disposition, VisualQualityDisposition.BLOCKED)
        joined = ' '.join(decision.blockers)
    ...
    def test_dense_copy_face_overlap_and_identity_placeholder_are_blocked(self):
        decision = self.gate.evaluate(self.evidence(
            dense_copy_used=True,
            readable_text_over_protected_face=True,
            exact_identity_placeholder_used=True,
        ))
        self.assertEqual(decision.disposition, VisualQualityDisposition.BLOCKED)
