...
def _build(result: dict[str, object] | None = None) -> dict[str, object]:
    return source_bound.build_artifact_ready_manifest(result or _verified_result(), workflow_run_id=RUN_ID, workflow_run_attempt=RUN_ATTEMPT)
...
class ArtifactReadyManifestTests(unittest.TestCase):
    ...
    def test_manifest_binds_source_resource_lock_png_models_and_workflow_run(self) -> None:
        manifest = _build()
        ...
        self.assertEqual(source_bound.artifact_ready_filename(RUN_ID, RUN_ATTEMPT), f"first-genuine-golden-v6-artifact-ready-run-{RUN_ID}-attempt-{RUN_ATTEMPT}.json")

    def test_rejects_unverified_incomplete_or_network_enabled_evidence(self) -> None:
        for patch in (
            {"source_commit_verified": False}, {"cost_mode": "network"}, {"evidence_files_verified": 9},
            {"evidence_semantics_verified": False}, {"local_only_model_receipts_verified": False},
            {"network_download_authorized": True}, {"local_files_only": False},
            {"qwen_model_revision": "f" * 40}, {"flux_model_revision": "e" * 40},
        ):
            with self.subTest(patch=patch):
                with self.assertRaises(RuntimeError): _build({**_verified_result(), **patch})
    ...
    def test_rejects_invalid_or_missing_run_identity(self) -> None:
        for run_id, attempt in ((None, RUN_ATTEMPT), ("", RUN_ATTEMPT), (0, RUN_ATTEMPT), (RUN_ID, 0), (RUN_ID, "01")):
            with self.assertRaisesRegex(RuntimeError, "WORKFLOW_RUN"):
                source_bound.build_artifact_ready_manifest(_verified_result(), workflow_run_id=run_id, workflow_run_attempt=attempt)
    ...
    def test_rejects_any_downstream_authority_drift(self) -> None:
        for field in ("human_visual_review_approved", "golden_quality_approved", "publication_ready", "seeds_2_to_4_authorized"):
            result = _verified_result(); result[field] = True
            with self.assertRaisesRegex(RuntimeError, "ILLEGAL_AUTHORITY"): _build(result)

    def test_ready_manifest_replay_rejects_stale_run_id_or_attempt(self) -> None:
        manifest = _build()
        source_bound.verify_artifact_ready_manifest(manifest, _verified_result(), expected_workflow_run_id=RUN_ID, expected_workflow_run_attempt=RUN_ATTEMPT)
        for run_id, attempt in ((RUN_ID + 1, RUN_ATTEMPT), (RUN_ID, RUN_ATTEMPT + 1)):
            with self.assertRaisesRegex(RuntimeError, "REPLAY_MISMATCH"):
                source_bound.verify_artifact_ready_manifest(manifest, _verified_result(), expected_workflow_run_id=run_id, expected_workflow_run_attempt=attempt)

    def test_writes_run_specific_manifest_atomically_as_valid_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            destination = Path(tmp) / "phase18_gpu_smoke" / source_bound.artifact_ready_filename(RUN_ID, RUN_ATTEMPT)
            manifest = source_bound.write_artifact_ready_manifest(destination, _verified_result(), workflow_run_id=RUN_ID, workflow_run_attempt=RUN_ATTEMPT)
    ...
    def test_replays_existing_uploaded_manifest_without_rewriting_it(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            directory = Path(tmp) / "phase18_gpu_smoke"; directory.mkdir(parents=True)
            receipt = directory / "first-genuine-golden-v6-resource-lock.json"; receipt.write_text("{}\n", encoding="utf-8")
            ...
            verified_path = source_bound.replay_existing_artifact_ready_manifest(receipt, _verified_result(), workflow_run_id=RUN_ID, workflow_run_attempt=RUN_ATTEMPT)
    ...
    def test_existing_ready_replay_rejects_missing_tampered_or_wrong_run_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(RuntimeError, "MANIFEST_MISSING"):
                source_bound.replay_existing_artifact_ready_manifest(receipt, _verified_result(), workflow_run_id=RUN_ID, workflow_run_attempt=RUN_ATTEMPT)
            wrong_run = directory / source_bound.artifact_ready_filename(RUN_ID + 1, RUN_ATTEMPT); wrong_run.write_text(json.dumps(_build()) + "\n", encoding="utf-8")
            with self.assertRaisesRegex(RuntimeError, "MANIFEST_MISSING"):
                source_bound.replay_existing_artifact_ready_manifest(receipt, _verified_result(), workflow_run_id=RUN_ID, workflow_run_attempt=RUN_ATTEMPT)
            ready = directory / source_bound.artifact_ready_filename(RUN_ID, RUN_ATTEMPT)
            tampered = _build(); tampered["network_download_authorized"] = True; ready.write_text(json.dumps(tampered) + "\n", encoding="utf-8")
            with self.assertRaisesRegex(RuntimeError, "REPLAY_MISMATCH"):
                source_bound.replay_existing_artifact_ready_manifest(receipt, _verified_result(), workflow_run_id=RUN_ID, workflow_run_attempt=RUN_ATTEMPT)
