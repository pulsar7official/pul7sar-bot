...
class UploadTransportAttestationTests(unittest.TestCase):
    def test_attestation_binds_upload_outputs_offline_provenance_and_keeps_authority_closed(self):
        result = build_attestation(metadata(), readiness(), expected_source_sha=SHA, workflow_run_id=77, workflow_run_attempt=1, upload_artifact_id=123, upload_artifact_digest=DIGEST)
    ...
    def test_mismatched_upload_id_is_rejected(self):
        with self.assertRaisesRegex(RuntimeError, "UPLOAD_ARTIFACT_ID_MISMATCH"):
            build_attestation(metadata(), readiness(), expected_source_sha=SHA, workflow_run_id=77, workflow_run_attempt=1, upload_artifact_id=124, upload_artifact_digest=DIGEST)

    def test_mismatched_upload_digest_is_rejected(self):
        with self.assertRaisesRegex(RuntimeError, "ARTIFACT_DIGEST_MISMATCH"):
            build_attestation(metadata(), readiness(), expected_source_sha=SHA, workflow_run_id=77, workflow_run_attempt=1, upload_artifact_id=123, upload_artifact_digest="c" * 64)

    def test_noncanonical_upload_ids_are_rejected(self):
        for value in (0, -1, "01", True):
            with self.subTest(value=value):
                with self.assertRaises(RuntimeError):
                    build_attestation(metadata(), readiness(), expected_source_sha=SHA, workflow_run_id=77, workflow_run_attempt=1, upload_artifact_id=value, upload_artifact_digest=DIGEST)

    def test_incomplete_evidence_readiness_is_rejected(self):
        ready = readiness(); ready["evidence_files_verified"] = 9
        with self.assertRaisesRegex(RuntimeError, "READINESS_EVIDENCE_NOT_VERIFIED"):
            build_attestation(metadata(), ready, expected_source_sha=SHA, workflow_run_id=77, workflow_run_attempt=1, upload_artifact_id=123, upload_artifact_digest=DIGEST)

    def test_network_authority_readiness_is_rejected(self):
        ready = readiness(); ready["network_download_authorized"] = True
        with self.assertRaisesRegex(RuntimeError, "READINESS_LOCAL_ONLY_NOT_VERIFIED"):
            build_attestation(metadata(), ready, expected_source_sha=SHA, workflow_run_id=77, workflow_run_attempt=1, upload_artifact_id=123, upload_artifact_digest=DIGEST)
