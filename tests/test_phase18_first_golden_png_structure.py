...
def _png(*, trailing: bytes = b"", corrupt_crc: bool = False, decoded: bytes | None = None,
         interlace: int = 0, bit_depth: int = 8, color_type: int = 2) -> bytes:
    channels = {0: 1, 2: 3, 3: 1, 4: 2, 6: 4}[color_type]
    if decoded is None:
        decoded = b"\x00" + bytes((channels * bit_depth + 7) // 8)
...
def _manifest(root: Path, png: Path) -> Path:
    ...
    payload = {
        ...
        "branch": "phase18/story-intelligence", "candidate": 1, "cost_mode": "$0-local",
        "offline_only": True, "source_commit_sha": "a" * 40, "png_sha256": digest,
...
class FirstGoldenPngStructureTests(unittest.TestCase):
    ...
    def test_valid_png_structure_and_canonical_encoding_are_verified(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png())
            result = verify(manifest_path=_manifest(root, png), repo_root=root)
            self.assertTrue(result["png_structure_verified"])
            self.assertTrue(result["canonical_encoding_verified"])
            self.assertEqual(result["canonical_encoding"], "RGB8_TRUECOLOUR_NON_INTERLACED")
            self.assertEqual((result["bit_depth"], result["color_type"], result["channels"], result["bits_per_pixel"]), (8, 2, 3, 24))
    ...
    def test_rgba8_is_rejected_by_critical_path_canonical_gate(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png(color_type=6))
            with self.assertRaisesRegex(RuntimeError, "CANONICAL_ENCODING_DRIFT"):
                verify(manifest_path=_manifest(root, png), repo_root=root)

    def test_rgb16_is_rejected_by_critical_path_canonical_gate(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png(bit_depth=16))
            with self.assertRaisesRegex(RuntimeError, "CANONICAL_ENCODING_DRIFT"):
                verify(manifest_path=_manifest(root, png), repo_root=root)

    def test_trailing_bytes_are_rejected(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png(trailing=b"unexpected"))
            with self.assertRaisesRegex(RuntimeError, "TRAILING_BYTES"):
                verify(manifest_path=_manifest(root, png), repo_root=root)

    def test_crc_drift_is_rejected(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png(corrupt_crc=True))
            with self.assertRaisesRegex(RuntimeError, "CRC_INVALID"):
                verify(manifest_path=_manifest(root, png), repo_root=root)

    def test_short_decoded_scanline_is_rejected(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png(decoded=b"\x00\x00\x00"))
    ...
    def test_invalid_scanline_filter_byte_is_rejected(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png(decoded=b"\x05\x00\x00\x00"))
    ...
    def test_interlaced_candidate_is_fail_closed_until_adam7_is_fully_verified(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png(interlace=1))
    ...
    def test_manifest_authority_drift_is_rejected(self) -> None:
        with self._with_root() as tmp:
            root = Path(tmp); png = root / "candidate.png"; png.write_bytes(_png())
            path = _manifest(root, png); payload = json.loads(path.read_text(encoding="utf-8"))
            payload["publication_ready"] = True; path.write_text(json.dumps(payload), encoding="utf-8")
    ...
    def test_workflow_runs_png_structure_gate_before_snapshot_replay_and_packaging(self) -> None:
        ...
        self.assertLess(gate, snapshot); self.assertLess(snapshot, package); self.assertLess(package, upload)
