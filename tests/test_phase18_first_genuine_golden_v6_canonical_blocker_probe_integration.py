...
class FirstGenuineGoldenV6CanonicalBlockerProbeIntegrationTests(unittest.TestCase):
    def test_canonical_workflow_records_probe_before_cuda_preflight(self) -> None:
    ...
    def test_canonical_cuda_preflight_is_native_bf16_zero_cost_and_offline_only(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
    ...
    def test_blocker_receipt_is_in_always_uploaded_evidence_tree(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        upload_step = "Upload genuine Golden v6 Candidate 1 evidence"

        self.assertIn(upload_step, text)
        upload_index = text.index(upload_step)
        tail = text[upload_index:]
