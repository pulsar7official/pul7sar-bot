...
# ما يتعارض بصرياً مع علامتنا التجارية. أبقيت فقط مصادر صور ثابتة نظيفة.
...
# تحويلها لمقاس أفقي 1280x720 بطريقة الـ Cover.
...
def search_wikipedia_image(query: str):
    ...
    # (require_landscape=True داخل download_image) وينتقل الكود لمصدر
    # DDGS الذي يعطي غالباً صور أكشن أفقية أنسب للتصميم.
...
def search_ddgs_image(query: str):
    ...
    ومفلتر بصلة عنوان النتيجة بالاستعلام (is_relevant_result) لتفادي صور حقيقية
    المصدر لكنها عن موضوع مختلف تماماً (مثل صورة كريكيت على خبر كرة قدم).
    """
    def _search():
        with DDGS() as ddgs:
    ...
    # ترتيب الأولوية: (موثوق + مرتبط بالموضوع) ثم (موثوق فقط) كحل وسط،
    # ولا نلجأ لنتائج غير موثوقة إطلاقاً حفاظاً على واقعية الصورة.
    candidates = trusted_relevant if trusted_relevant else trusted
    if not candidates:
        LOG.warning("لا نتائج موثوقة أو مرتبطة بالموضوع لـ '%s'.", query)
        return None
