...
# كلمات وظيفية (Stopwords) تُستبعد من مقارنة تشابه العناوين لتفادي false positives
STOPWORDS = {
...
# ربط كل ثيم بلونه وبشعاره المناسب معاً (يحل مشكلة عدم تطابق الشعار مع اللون)
STRIPE_THEMES = [
...
# ==============================================================================
# أدوات مساعدة (retry + timeout لكل الشبكة)
# ==============================================================================
...
# ==============================================================================
# سجل منع التكرار (يحافظ على الترتيب الحقيقي، بدون استخدام set عشوائي)
# ==============================================================================
...
def save_history(history):
    # القص من البداية (الأقدم) للحفاظ على الترتيب الصحيح، بدل list(set(...)) العشوائي
...
def is_topic_repeated(new_title: str, posted_titles: list) -> bool:
    ...
    for pt in posted_titles:
        ...
        # نسبة تشابه حقيقية بدل عدد ثابت، لتفادي تطابقات وهمية بين عناوين مختلفة
...
def fetch_articles(posted_links_set, posted_titles):
    ...
    for url in RSS_URLS:
        ...
        for entry in feed.entries[:12]:
            ...
            # استبعاد أي مقال بدون تاريخ نشر موثوق (بدل نشره بدون تحقق)
...
def sanitize_news_text(text: str) -> str:
    # فقط إزالة رموز الماركداون — بدون حذف كلمات كامل الجملة (كان يكسر المعنى سابقاً)
...
def search_ddgs_image(query: str):
    ...
    random.shuffle(results)  # منع تكرار نفس الصورة بالضبط في كل مرة
    for r in results:
        img_url = r.get("image")
        if not img_url:
            continue
...
def build_final_image(base_img, stripe_hex, logo_key):
    ...
    # شريط العلامة التجارية — ارتفاع واضح (~8% من الطول بدل 10px بالكاد مرئية)
    stripe_height = 58
    hex_color = stripe_hex.lstrip("#")
    rgb_color = tuple(int(hex_color[i:i + 2], 16) for i in (0, 2, 4))
    draw.rectangle([(0, 720 - stripe_height), (1280, 720)], fill=rgb_color)

    # الشعار مرتبط بنفس ثيم اللون المحسوب (يحل مشكلة عدم التطابق)
