...
def extract_entities(article: dict) -> dict:
    ...
    LOG.info("🔎 كيانات الخبر: لاعب=%s | نادٍ=%s | رياضة=%s | استعلام=%s",
             player, club, sport, query)
    return {"player": player, "club": club, "sport": sport, "query": query}
...
def layer6_editorial_style(text: str) -> str:
    ...
    الكامل على كل حقيقة/رقم/اسم كما هو. لا يقصّر النص جذرياً هنا -- تحويله لصيغة
    "خبر مختصر + تعليق أول" مهمة منفصلة (المرحلة 4، لم تبدأ بعد).
    """
    prompt = (
...
def ensure_perfect_arabic(text: str) -> str:
    """
    ينفّذ سلسلة المعالجة السباعية كاملة. كل طبقة تُصلح لا ترفض، والبوابة النهائية
    (Layer 7) وحدها تقرر الرفض -- وفقط بعد استنفاد كل محاولات الإصلاح أعلاه.
    """
    if not text:
...
def get_best_image_for_article(article: dict) -> Image.Image:
    ...
    image_sources = [
        ...
        ("DDGS Trusted", lambda: search_ddgs_image(entities, entities["query"], require_relevance=True)),
    ]
