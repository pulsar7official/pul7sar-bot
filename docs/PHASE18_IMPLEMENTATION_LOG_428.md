...
The remaining integration gap was that the JIT execution entrypoint did not invoke that probe automatically before entering the heavier offload/resource/generation chain.
...
The lower-level `run()` function was intentionally left independent of this host probe so existing deterministic/unit replay tests can continue exercising the JIT evidence logic without requiring a CUDA host. The authoritative workflow CLI path receives the new guard.
...
- the probe executes before the heavier JIT `run()` path;
...
Before CS428, an apparently compatible self-hosted runner could enter the heavier JIT/offload chain and only later reveal that a required CUDA/BF16/local-model-cache prerequisite was absent. After CS428, the actual JIT CLI first produces one deterministic blocker receipt and stops before the heavier chain when any prerequisite is missing.

Because the receipt is written under `output/phase18_gpu_smoke/` and the existing workflow uploads that tree with `if: always()`, a failed first-Golden attempt can preserve the exact blocker evidence instead of leaving only an opaque failed step.
