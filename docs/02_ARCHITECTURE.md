...
The Pipeline is the orchestration layer of the Visual Engine.

It coordinates every rendering request from the validated input until the final rendered output.

The Pipeline never performs rendering itself.
...
- Resolving assets.
- Resolving fonts.
- Selecting the requested Template.
- Creating the RenderContext.
- Creating the Renderer.
- Injecting the Canvas implementation.
- Executing the rendering process.
- Returning the rendered result.

The Pipeline owns the rendering lifecycle.

---

## Rendering Lifecycle

Every rendering request follows exactly this sequence.

1. Receive ValidatedPayload.
2. Resolve configuration.
3. Resolve assets.
4. Resolve fonts.
5. Select Template.
6. Build RenderContext.
7. Request Layers from Template.
8. Create Renderer.
9. Inject Canvas implementation.
10. Execute Renderer.
11. Receive rendered image.
12. Pass image to Exporter.
13. Return exported result.

The Pipeline must never skip any step.

---

## Dependency Management

The Pipeline is responsible for dependency injection.

The Pipeline creates or receives:
...
Subsystems must never construct each other directly.
...
Pipeline converts subsystem failures into a consistent engine failure.

Subsystem exceptions may propagate unchanged if already derived from VisualEngineError.

Unexpected exceptions should be wrapped into RenderingError.
...
Pipeline must execute exactly one rendering request at a time.

Pipeline must never cache rendering state.

Pipeline must never modify Layer objects.

Pipeline must never modify RenderContext after creation.

---

## Architectural Rule

The Pipeline is the only component allowed to coordinate multiple subsystems.

No other subsystem may orchestrate the rendering lifecycle.
